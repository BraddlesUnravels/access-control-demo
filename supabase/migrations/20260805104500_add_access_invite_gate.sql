...
create table public.access_invites (
  ...
  constraint access_invites_code_hash_not_blank
    check (char_length(trim(code_hash)) > 0),

  constraint access_invites_label_not_blank
    check (char_length(trim(label)) > 0),

  constraint access_invites_use_count_non_negative
    check (use_count >= 0)
);
...
create table public.access_visits (
  ...
  invite_id uuid not null
    references public.access_invites (id)
    on delete cascade,
...
revoke all on table public.access_invites
from anon, authenticated, service_role;

revoke all on table public.access_visits
from anon, authenticated, service_role;
...
grant select, insert
on table public.access_invites
to service_role;
...
begin
  if p_code_hash is null
    or char_length(trim(p_code_hash)) = 0
  then
    return query
    select
      null::uuid,
...
revoke all
on function public.redeem_access_invite(text)
from public;
