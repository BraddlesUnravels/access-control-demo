...
Database migrations under `supabase/migrations/` are the executable source of
truth. The checked-in `supabase/schema.sql` file is a generated schema snapshot
and must remain synchronized with those migrations.

When changing the database schema:

1. Add or update a migration.
2. Reset the local database so the migration is applied:

   ```bash
   npm run infra:reset
   ```

3. Regenerate the TypeScript database types:

   ```bash
   npm run db:types
   ```

4. Regenerate or verify the schema snapshot as appropriate:
...
Commit the migration, schema snapshot, and generated types together. Do not
edit `lib/supabase/database.types.ts` or `supabase/schema.sql` manually.
...
Database migrations are the executable source of truth. `supabase/schema.sql` is
a generated reference snapshot of the application-owned `public` and `private`
schemas; it includes the access-gate database objects. Objects owned by Supabase's
`auth` schema, including the trigger on `auth.users`, remain defined by the
migrations rather than this schema-filtered dump.

After adding a migration, regenerate the snapshot from a clean local database:

```bash
npm run schema:generate
```

Check that the snapshot still matches a running local database with all
migrations applied:
...
The container-stage workflow starts a fresh Supabase database, which validates
migration syntax, then runs `schema:check` to prevent snapshot drift.
...
| `npm run format:sql`      | Format Supabase SQL files with `pg_format`             |
...
| `npm run schema:generate` | Rebuild and dump the application schema snapshot       |
| `npm run schema:check`    | Check the schema snapshot against local Supabase       |
